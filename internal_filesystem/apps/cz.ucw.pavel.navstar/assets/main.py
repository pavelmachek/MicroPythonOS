...
class badUI:
    ...
    - clear() wipes the canvas buffer
    - text/line/circle/fill_circle draw directly to canvas
    - update() is a no-op (LVGL flush is driver-based)
    - 4 on-screen buttons provide input events
    """
    ...
    def __init__(self):
        ...
        # Reserve space for a bottom button bar
        self.margin = 2
        self.bar_h = 26
        ...
        self.canvas.set_size(self.W, self.draw_h)

        # Canvas buffer: TRUE_COLOR is simplest
        # (LVGL needs a bytearray; size depends on color format)
        #
        # For LV_COLOR_DEPTH=16 (common): 2 bytes per pixel
        # For LV_COLOR_DEPTH=32: 4 bytes per pixel
        #
        ...
        self._line_dsc.color = lv.color_white()
        ...
        self._label_dsc.color = lv.color_white()
        self._label_dsc.font = lv.font_montserrat_14

        # One-shot button press flags
        self._ev_next = False
        self._ev_rec = False
        self._ev_nav = False
    ...
    def line(self, x1, y1, x2, y2):
        ...
        self.canvas.draw_line(p1, p2, self._line_dsc)
    ...
    def fill_circle(self, x, y, r):
        ...
        self.canvas.draw_rect(a, self._fill_dsc)

    def update(self):
        # No-op for LVGL canvas
        pass
